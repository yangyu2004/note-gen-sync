...
`px01 `|距边外右侧设置|`-rightmargin `|| `px01 `|距边外左侧设置|`-leftmargin `|| `px01 `|距边外底部设置|`-bottommargin `|| `px01 `|距边外顶部设置|`-topmargin `|| `px02px01 `/`px02 `|）值方向多个接受可（距边外设置|`margin `|| `000000#`/`red `|颜色框边设置|`-colorborder `|| `ottedd `/`ashedd `/`solid `|样式框边设置|`-styleborder `|| `px2 `|宽度框边设置|`-widthborder `|| `blacksolidpx2 `|）值属性多个接受可（框边设置|`border `|| `px01 `|距边内右侧设置|`-rightpadding `|| `px01 `|距边内左侧设置|`-leftpadding `|| `px01 `|距边内底部设置|`-bottompadding `|| `px01 `|距边内顶部| 设置 `-toppadding `|| `px02px01 `/`px02 `|）值方向多个接受可（距边内设置|`padding `|| `auto `/`px001 `|的高度区域内容元素设置|`height `|| `%001 `/`px002 `|宽度的区域内容元素设置|| `     | width |`      |
| ------ | ----- | ------ |
| 值示例 | 作用  | 名属性 |

隔开# CSS 核心基础：语法、引入方式与常用属性
...
空间，用于控制元素内部的空白。 **边框（Border）**：围绕内容和内边距的边框，用于装饰元素。 **外边距（Margin）**：元素外部的空白区域，用于与其他元素
------------------------------------------------------------------------------------------------------------------------------------------
...
### 浮动的三大特性情况下，CSS 的 `width` 和 `height` 只包括**内容区域**，不包括内边距、边框和外边距。如果你想让 `width` 和 `height` 包含内边距和边框，可以使用 `box-sizing: border-box` 属性。
...
.box {
  ...
  box-sizing: border-box; /* 默认值为 content-box */
}
```
...
通过合理使用盒子模型，可以更精确地控制网页元素的布局和外观。| 部分 | 说明     |
| ---- | -------- |
|      | 内容区域 |
...
CSS 盒子模型是用于描述网页元素布局和外观的基本概念，它由**内容区域**、**内边距（padding）**、**边框（border）**和**外边距（margin）**四部分组成。

### 1. 盒子模型结构

每个 HTML 元素都可以看作一个矩形盒子，包含以下部分：

- **内容区域（Content）**：元素实际显示的内容，如文字、图片等。 **内边距（Padding）**：内容区域与边框
  -------------------------------------------------------------------------------------------
